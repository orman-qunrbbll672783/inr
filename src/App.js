...
const CompanyNameScreen = ({ onNext, onBack, companyName, setCompanyName, selectedRole }) => {
    ...
    return (<><BackButton onClick={onBack} /><OnboardingWrapper title="Company Name" step={2} totalSteps={isInsuranceCompany ? 3 : 4}><form onSubmit={onNext} className="space-y-6"><p className="text-gray-600">{isInsuranceCompany ? 'What is the name of your insurance company?' : 'What is the name of your leasing company?'}</p>{isInsuranceCompany && (<div className="mb-4"><p className="text-sm text-gray-500 mb-3">Popular insurance companies:</p><div className="grid grid-cols-2 gap-2">{popularInsuranceCompanies.map((company) => (<button key={company} type="button" onClick={() => setCompanyName(company)} className={`p-3 text-sm rounded-lg border transition-all duration-200 ${companyName === company ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 hover:border-emerald-300 hover:bg-emerald-50'}`}>{company}</button>))}</div><div className="flex items-center my-4"><div className="flex-1 border-t border-gray-200"></div><span className="px-3 text-xs text-gray-400">OR</span><div className="flex-1 border-t border-gray-200"></div></div></div>)}<div><label htmlFor="companyName" className="sr-only">Company Name</label><input id="companyName" type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} required className="mt-1 block w-full px-4 py-3 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-lg text-center" placeholder={isInsuranceCompany ? 'e.g., Your Insurance Company' : 'e.g., Acme Leasing Inc.'} /></div><button type="submit" disabled={!companyName} className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed ${isInsuranceCompany ? 'bg-emerald-600 hover:bg-emerald-700 focus:ring-emerald-500 disabled:bg-emerald-400' : 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 disabled:bg-indigo-400'}`}>Continue</button></form></OnboardingWrapper></>);
};
...
// --- Messaging Modal Component (Unchanged) ---
const MessagingModal = ({ isOpen, onClose, inquiry, user, profileData, messages, onSendMessage }) => {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4 animate-fade-in"><div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl h-[80vh] flex flex-col"><header className="p-4 border-b flex justify-between items-center"><h2 className="text-lg font-bold text-gray-800">Conversation with {otherPartyName}</h2><button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-2xl">&times;</button></header><main className="flex-1 p-4 overflow-y-auto bg-gray-50"><div className="space-y-4">{messages.map((msg) => (<div key={msg.id} className={`flex ${msg.senderId === user.uid ? 'justify-end' : 'justify-start'}`}><div className={`max-w-md p-3 rounded-xl ${msg.senderId === user.uid ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-800'}`}><p className="text-sm">{msg.text}</p><p className={`text-xs mt-1 ${msg.senderId === user.uid ? 'text-emerald-100' : 'text-gray-500'}`}>{msg.timestamp?.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>))}{messages.length === 0 && <p className="text-center text-gray-500">No messages yet.</p>}<div ref={messagesEndRef} /></div></main><footer className="p-4 border-t"><form onSubmit={handleSendMessage} className="flex space-x-2"><input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type your message..." className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" /><button type="submit" className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition-colors disabled:bg-emerald-300" disabled={!newMessage.trim()}>Send</button></form></footer></div></div>);
};
...
const DashboardScreen = ({ user, profileData, setAlert, notifications, onClearNotifications }) => {
    ...
    const sendMessage = async (inquiryId, messageText, recipientId) => {
        ...
        await createNotification(recipientId, `New message from ${profileData.companyName} regarding inquiry.`);
    };
                    ...
                    <div className="space-y-4">
                        {loadingHistory ? <LoadingSpinner /> : sentInquiries.length > 0 ? sentInquiries.map(inquiry => (
                             <div key={inquiry.id} className="bg-white rounded-xl p-4 border border-gray-200/80 flex flex-col sm:flex-row justify-between sm:items-center space-y-3 sm:space-y-0">
                                ...
                                <button onClick={() => openMessaging(inquiry)} className="px-4 py-2 bg-emerald-100 text-emerald-800 text-sm font-medium rounded-lg hover:bg-emerald-200 transition-colors w-full sm:w-auto">Messages</button>
                            </div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50 font-sans pb-20 sm:pb-0">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                    <div className="flex justify-between items-center h-16">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="relative">
                                <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center space-x-3 p-2 rounded-xl hover:bg-gray-100/80 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500"><div className="w-9 h-9 bg-gradient-to-r from-emerald-500 to-green-500 rounded-xl flex items-center justify-center shadow-md"><UserIcon className="w-5 h-5 text-white" /></div><div className="text-left hidden sm:block"><p className="text-sm font-semibold text-gray-900">{user.displayName || 'User'}</p><p className="text-xs text-gray-500">{profileData.companyName}</p></div></button>
            ...
            <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-t-lg flex justify-around z-40">
                <button onClick={() => setActiveView('new_inquiry')} className={`flex flex-col items-center justify-center w-full pt-2 pb-1 ${activeView === 'new_inquiry' ? 'text-indigo-600' : 'text-gray-500'}`}><PlusCircleIcon className="w-6 h-6 mb-1" /><span className="text-xs">New Inquiry</span></button>
                <button onClick={() => setActiveView('history')} className={`flex flex-col items-center justify-center w-full pt-2 pb-1 ${activeView === 'history' ? 'text-indigo-600' : 'text-gray-500'}`}><HistoryIcon className="w-6 h-6 mb-1" /><span className="text-xs">History</span></button>
            </nav>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50">
            ...
            <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/80 sticky top-0 z-30">
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                    <div className="flex justify-between items-center h-16">
                        <div className="flex items-center space-x-3"><div className="w-8 h-8 bg-gradient-to-r from-emerald-600 to-green-600 rounded-lg flex items-center justify-center shadow-md"><span className="text-white font-bold text-sm">IP</span></div><h1 className="text-lg font-bold text-gray-900 hidden sm:block">Insurance Dashboard</h1></div>
                        <div className="flex items-center space-x-4">
                            <div className="relative">
                                <button onClick={() => { setShowNotifications(p => !p); if (!showNotifications) { onClearNotifications(); } }} className="relative text-gray-600 hover:text-gray-800"><BellIcon className="w-6 h-6" />{notifications.filter(n => !n.read).length > 0 && (<span className="absolute -top-1 -right-1 flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span></span>)}</button>
                                {showNotifications && <NotificationsPanel notifications={notifications} onClear={onClearNotifications} />}
                            ...
                            <div key={inquiry.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                ...
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 pt-4 border-t border-gray-200 gap-3">
                                    <div className="flex items-center space-x-3">
                                        {inquiry.status === 'pending' && (<><button onClick={() => handleRespond(inquiry)} className="bg-emerald-600 text-white py-2 px-4 rounded-xl font-medium hover:bg-emerald-700 transition-all">Respond</button><button onClick={() => handleReject(inquiry)} className="bg-white border border-gray-300 text-gray-800 py-2 px-4 rounded-xl font-medium hover:bg-gray-100 transition-all">Reject</button></>)}
                                    </div>
                                    <button onClick={() => openMessaging(inquiry)} className="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors w-full sm:w-auto">Messages</button>
                                </div>
                            </div>
                        )))}
                    </div>
